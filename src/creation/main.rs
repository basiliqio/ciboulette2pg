...
) -> Result<Option<(&'a str, &'a str)>, Ciboulette2SqlError> {
    match relationships.get(to_type_alias) {
        Some(rel_obj) => match rel_obj.data() {
            Some(CibouletteResourceIdentifierSelector::One(rel_id)) => Ok(Some((opt.key().as_str(), rel_id.id()))),
...
pub fn gen_query_insert_main<'a>(
    store: &'a CibouletteStore,
    req: &'a CibouletteCreateRequest<'a>,
) -> Result<(String, Vec<quaint::Value<'a>>), Ciboulette2SqlError> {
    ...
    while let Some((edge_index, node_index)) = walker.next(&store.graph()) {
        ...
        // Get the edge weight
        {
            ...
            if let Some((k, v)) = insert_relationship_single(
                ...
                insert_stmt = insert_stmt.value(k, v); // Insert the relationship
            }
        }
    }
    Ok(Postgres::build(insert_stmt.build().returning(returning))?)
}
