...
fn relationship_external() {
    ...
    let request = req_builder.build(&ciboulette_store).unwrap();
    let ciboulette_request = CibouletteUpdateRequest::try_from(request).unwrap();
    let err: Ciboulette2SqlError = Ciboulette2PostgresBuilder::gen_update_main(
        &ciboulette_store,
        &table_store,
        &ciboulette_request,
    )
    ...
    assert_eq!(
        matches!(err, Ciboulette2SqlError::UpdateManyRelationships),
        true
    );
}

#[test]
fn relationship_single_with_multiple_values() {
    let ciboulette_store = gen_bag();
    let table_store = gen_table_store();
    let parsed_url =
        Url::parse("http://localhost/peoples/6720877a-e27e-4e9e-9ac0-3fff4deb55f2").unwrap();
			...
			"relationships":
			{
				"favorite_color":
				{
					...
					[
						...
						{
							"id": "b0b0d3af-2508-47c6-a7ba-4a34f68dbddd",
							"type": "favorite_color"
						}
					]
				}
    ...
    assert_eq!(
        matches!(err, Ciboulette2SqlError::RequiredSingleRelationship(x) if x == "favorite_color"),
        true
    );
}
